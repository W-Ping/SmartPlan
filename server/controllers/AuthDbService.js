const debug = require('debug')('[AuthDbService]')
const uuidGenerator = require('uuid/v4')
const moment = require('moment')
const ERRORS = require('./constants').ERRORS_BIZ
...
function saveUserInfo (userInfo, skey, session_key) {
    ...
    return mysql('cSessionInfo').count('open_id as hasUser').where({
        ...
        .catch(e => {
            debug('%s: %O', ERRORS.DBERR.BIZ_ERR_WHEN_INSERT_TO_DB, e)
            throw new Error(`${ERRORS.DBERR.BIZ_ERR_WHEN_INSERT_TO_DB}\n${e}`)
        })
}
...
function getUserInfoBySKey (skey) {
    if (!skey) throw new Error(ERRORS.DBERR.BIZ_ERR_NO_SKEY_ON_CALL_GETUSERINFOFUNCTION)
...
function getUserInfoByOpenId (openId) {
    if (!openId) throw new Error(ERRORS.DBERR.BIZ_ERR_NO_SKEY_ON_CALL_GETUSERINFOFUNCTION)
